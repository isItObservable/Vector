...
sed -i "s,CLUSTER_ID_TOREPLACE,$CLUSTERID," vector/values.yaml
sed -i "s,CLUSTER_ID_TOREPLACE,$CLUSTERID," vector/values_logsonly.yaml
...
kubectl create secret generic dynatrace  --from-literal=dynatrace_oltp_url="$DTURL" --from-literal=clustername="$CLUSTERNAME"  --from-literal=clusterid=$CLUSTERID  --from-literal=dt_api_token="$DTTOKEN" -n vector
